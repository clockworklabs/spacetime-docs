...
Clients can request a new identity and token via [the `/identity POST` HTTP endpoint](identity#identity-post.).

Alternately, a new identity and token will be generated during an anonymous connection via the [WebSocket API](ws.), and passed to the client as [an `IdentityToken` message](ws#identitytoken.).
